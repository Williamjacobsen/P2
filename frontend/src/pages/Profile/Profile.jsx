  ...
  return (
    <>
      <div className="profile-info">
        ...
        <br />
        <button onClick={signOutAllDevices}>Sign out on all devices</button>
        <br />
        <Modal
        ...
        {vendor !== null && (
          <>
            ...
            />
            <br />
            <b>Public vendor description: </b>"{vendor.Description}"
            <br />
            <Modal
...
async function requestProfileModification(password, propertyName, newValue) {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/profile/modify`,
      {
        method: "PUT",
        credentials: "include", // Ensures cookies are sent with the request
        headers: {
    ...
    if (!response.ok) {
      if (data.error === "Access token is invalid") {
        await requestAccessToken();
        return await requestProfileModification(
          password,
          propertyName,
          newValue
        );
...
async function requestVendorModification(password, propertyName, newValue) {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/vendor/modify`,
      {
        method: "PUT",
        credentials: "include", // Ensures cookies are sent with the request
        headers: {
    ...
    if (!response.ok) {
      if (data.error === "Access token is invalid") {
        await requestAccessToken();
        return await requestVendorModification(
          password,
          propertyName,
          newValue
        );
...
async function requestSignOut() {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/profile/sign-out-device`,
      {
        method: "POST",
        credentials: "include", // Ensures cookies are sent with the request
      }
...
async function requestSignOutAllDevices() {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/profile/sign-out-all-devices`,
      {
        method: "POST",
        credentials: "include", // Ensures cookies are sent with the request
      }
