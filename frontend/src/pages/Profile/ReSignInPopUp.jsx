      ...
      {isVisible && (
        <div className="window">
          ...
          <div className="window-content">
            Your sign in refresh token has expired. Please sign in again, or
            sign out.
            <br />
            If you sign in, the page will not reload, so do not worry about
            losing progress.
            ...
            <button className="button" onClick={signOut}>
              {" "}
              {/* signOut() also refreshes the page */}
              Sign out
            </button>
          </div>
          <br />
...
export async function requestAccessToken() {
  try {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/profile/generate-access-token`,
      {
        method: "POST",
