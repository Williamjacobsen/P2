...
export default function ProductCatalogue() {
  ...
  const [
    ...
    PreviousPageButton,
    NextPageButton,
  ] = usePages(shownProducts, productsPerPage);

  const location = useLocation();

  const genderOptionArray = ["Male", "Female"];
  ...
  const priceOptionArray = ["Highest Price", "Lowest Price"];
  ...
  const storeOptionArray = Array.from(
    ...
    new Set(
      ...
      loadedProducts.map((product) => product.StoreName)
    )
  );
  ...
  function sortProductsLogic(products) {
    ...
    if (priceSort === "Highest Price") {
      return products.sort((product1, product2) => {
        return loadedProducts[product2].Price - loadedProducts[product1].Price;
      });
    }
    if (priceSort === "Lowest Price") {
      return products.sort((product1, product2) => {
        return loadedProducts[product1].Price - loadedProducts[product2].Price;
      });
    }
    return products;
  }
  ...
  function filterProductsLogic(productIndices) {
    return productIndices.filter((index) => {
      ...
      return (
        ...
        (clothingTypeFilter === "" ||
          product.ClothingType === clothingTypeFilter) &&
        (search === "" ||
