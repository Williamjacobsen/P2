...
router.get("/get", [validateVendorID], async (req, res) => {
  try {
    ...
    try {
      ...
      if (!validationErrors.isEmpty()) {
        return res.status(400).json({
          error:
            "Input is invalid for the input: '" +
            validationErrors.array()[0].path +
            "'",
        }); // 400 = Bad request
      }
      // Get data from request
      ...
      await pool.query(
        `UPDATE p2.Vendor SET ${propertyName}='${newValue}' WHERE (ID='${vendorID}');`
      );
      // Send back response
...
router.get(
  ...
  [
    ...
    check("vendorid").bail().escape().notEmpty().isInt(),
  ],
  async (req, res) => {
    // Handle validation errors
    const validationErrors = validationResult(req);
    ...
    if (vendorid <= 0) {
      return res
        .status(400)
        .json({ message: "Vendor ID must be a positive integer" });
    }
...
async function getVendor(httpResponse, vendorID) {
  ...
  const [vendorRows] = await pool.query(
    `SELECT * FROM p2.Vendor WHERE ID='${vendorID}';`
  );
