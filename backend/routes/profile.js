...
router.post("/sign-in", [validateEmail, validatePassword], async (req, res) => {
  try {
    ...
    if (res._header === null) {
      // If _header !== null, then the response has already been handled someplace else
...
router.post(
  "/sign-out-device",
  [validateProfileRefreshToken, validateProfileAccessToken],
  async (req, res) => {
    ...
    try {
      ...
      await pool.query(
        `DELETE FROM p2.ProfileRefreshToken WHERE Token='${refreshToken}';`
      );
      // Send back response
      res.set("Cache-Control", "no-store"); // Prevents caching of the response (for security reasons).
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Remove refresh tokens from the server
      await pool.query(
        `DELETE FROM p2.ProfileRefreshToken WHERE ProfileID='${profileID}';`
      );
      // Send back response
      res.set("Cache-Control", "no-store"); // Prevents caching of the response (for security reasons).
      setSecureCookie(res, "profileRefreshToken", refreshToken, 0);
...
router.get("/get", [validateProfileAccessToken], async (req, res) => {
  try {
    // Handle validation errors
    handleValidationErrors(
      req,
      ...
      function (validationErrors) {
        if (
          validationErrors
            .array()
            .some((error) => error.path === "profileAccessToken")
        ) {
  ...
  async (req, res) => {
    try {
      ...
      let [profile] = await pool.query(
        `SELECT * FROM p2.Profile WHERE Email='${email}';`
      );
      if (Object.keys(profile).length !== 0) {
        return res
          .status(409)
          .json({ error: "Another profile already uses that email." }); // 409 = Conflict
      }
      // Check if the phone number is already used by an existing profile
      [profile] = await pool.query(
        `SELECT * FROM p2.Profile WHERE PhoneNumber='${phoneNumber}';`
      );
      if (Object.keys(profile).length !== 0) {
        return res
          .status(409)
          .json({ error: "Another profile already uses that phone number." }); // 409 = Conflict
      }
...
router.post(
  "/delete",
  [validatePassword, validateProfileAccessToken],
  async (req, res) => {
    try {
      // Handle validation errors
      handleValidationErrors(req, res, validationResult);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      handleValidationErrors(req, res, validationResult);
      // Get data from request
      let { password, propertyName, newValue } = req.body;
      const accessToken = req.cookies.profileAccessToken;
      ...
      switch (propertyName) {
        case "Email":
          [anotherProfileRows] = await pool.query(
            `SELECT * FROM p2.Profile WHERE Email='${newValue}';`
          );
      ...
      await pool.query(
        `UPDATE p2.Profile SET ${propertyName}='${newValue}' WHERE (ID='${profileID}');`
      );
      // Send back response.
      return res.status(201).json({}); // 201 = Created
    } catch (error) {
...
nodeSchedule.scheduleJob(rule, async function () {
  console.log(
    "SCHEDULE TASK STARTED: Deleting expired refresh tokens from database."
  );
  await pool.query(
    `DELETE FROM p2.ProfileRefreshToken WHERE ExpirationDateTime < NOW();`
  );
});
...
async function decodeRefreshToken(httpResponse, refreshToken) {
  try {
    ...
    const [unexpiredRefreshTokenRows] =
      await pool.query(`SELECT * FROM p2.ProfileRefreshToken 
...
export async function getProfile(httpResponse, accessToken) {
  ...
  const [profileRows] = await pool.query(
    `SELECT * FROM p2.Profile WHERE ID='${profileID}';`
  );
...
async function generateRefreshToken(httpRequest, profileID) {
  ...
  if (oldRefreshToken !== undefined) {
    const [oldTokenRows] = await pool.query(
      `SELECT * FROM p2.ProfileRefreshToken WHERE Token='${oldRefreshToken}';`
    );
  ...
  expirationDateTime.setDate(
    expirationDateTime.getUTCDate() + refreshTokenExpirationAgeInDays
  );
  if (shouldUpdateOldToken) {
    // Update existing token
    await pool.query(
      `UPDATE p2.ProfileRefreshToken 
    ...
    // Insert new token
    await pool.query(
      `INSERT INTO p2.ProfileRefreshToken 
    (ProfileID, ExpirationDateTime, Token) 
    VALUES (?, ?, ?)`,
      [profileID, expirationDateTime, newRefreshToken]
    ); // I've used the "?"-notation because else it does not pass in the dateTime correctly.
  }
...
async function generateAccessToken(httpResponse, refreshToken) {
  ...
  const [refreshTokenRows] = await pool.query(
    `SELECT * FROM p2.ProfileRefreshToken WHERE Token='${refreshToken}';`
  );
  ...
  const accessToken = jwt.sign(tokenPayload, accessTokenSecretKey, {
    expiresIn: accessTokenExpirationAge,
  });
  return accessToken;
}
